...
/**
 * 类 名 称：TableContext
 * 类 描 述：负责获取管理数据库所有表结构和类结构的关系，并可以根据表结构生成类结构
 * 创建时间：2019/4/29 10:35
 * 创建人：Mical
 */
public class TableContext {
    ...
    //将 po 的class 对象和表信息对象关联起来，便于重用
    public static Map<Class, TableInfo> poClassTableMap = new HashMap<>();

    //将空构造器私有化，阻止外部 new 出此类
    ...
    static {
        try {
            ...
            ResultSet tableRet = dbmd.getTables(JDBCUtils.getDBName(), "%", "%", new String[]{"TABLE"});
    ...
    public static void updateJavaPOFile() {
        ...
        for (TableInfo value : map.values()) {
        ...
        for (TableInfo value : tables.values()) {
            try {
                Class c = Class.forName(DBManager.getConf().getPoPackage()
                        + "." + StringUtils.firstChar2UpperCase(value.getTname()));
                poClassTableMap.put(c, value);
